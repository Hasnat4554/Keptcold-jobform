...
import { BusinessDetails, FaultDetails, ServiceType, SERVICE_OPTIONS } from '@/types/booking';
...
function PaymentForm({ businessDetails, faultDetails, serviceType, onBack, clientSecret }: Props & { clientSecret: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-gray-50 rounded-lg p-6 mb-6">
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium">{businessDetails.jobAddress || businessDetails.businessAddress}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600">Equipment:</span>
            <span className="font-medium">{faultDetails.equipmentType}</span>
          </div>
          <div className="flex justify-between">
      ...
      <div className="flex items-center justify-center gap-2 text-sm text-gray-600 pt-4">
        ...
        <span>Powered by <strong>Stripe</strong> - Secure Payment Processing</span>
      </div>
    </form>
  );
}
...
export default function Step4Payment(props: Props) {
  ...
  const selectedService = SERVICE_OPTIONS.find(s => s.id === props.serviceType);
  const totalPrice = selectedService ? selectedService.price : 0;
    ...
    <div className="bg-white rounded-lg shadow-lg p-8">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12">
          <svg className="animate-spin h-12 w-12 text-[#003366]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
        </div>
